...
public class ThumbnailatorUtils {
    ...
    public static void drawRectAndSave(ImagePointPair imagePointPair){
        try {
            File file = new File(imagePointPair.imagename);
            if (!file.exists()) {
               return ;
            }
            ...
            //描框
            for (ImagePointPair.PointPair pp : imagePointPair.pairList) {
                int startX = (int) (pp.pointUp.x * image.getWidth());
                int startY = (int) (pp.pointUp.y*image.getHeight());

                //计算长度
                int widthleg = (int) ((pp.pointDown.x - pp.pointUp.x) * image.getWidth());
                int heightleg = (int) ((pp.pointDown.y - pp.pointUp.y) * image.getHeight());

                //矩形框(原点x坐标，原点y坐标，矩形的长，矩形的宽)
                g.drawRect(startX,startY,widthleg,heightleg);
            }
            g.dispose();
            ResourceHandler.saveFile(imagePointPair.imagename,image);
        }catch (Exception e){
            e.printStackTrace();
        }
    }
